...
DATASETS = {
    ...
    "def": os.path.join(
        os.path.dirname(os.path.dirname(__file__)),
        "resources",
        "nonce.definitions.299.test",
    ),
    "l2": os.path.join(
        os.path.dirname(os.path.dirname(__file__)),
        "resources",
        "chimeras.dataset.l2.tokenised.test.txt",
    ),
    "l4": os.path.join(
        os.path.dirname(os.path.dirname(__file__)),
        "resources",
        "chimeras.dataset.l4.tokenised.test.txt",
    ),
    "l6": os.path.join(
        os.path.dirname(os.path.dirname(__file__)),
        "resources",
        "chimeras.dataset.l6.tokenised.test.txt",
    ),
}
...
def get_model_path(
    ...
    return os.path.join(
        outputdir,
        "{}.{}.alpha{}.neg{}.win{}.sample{}.epochs{}.mincount{}.size{}.model".format(
...
class Samples:  # pylint:disable=R0903
    ...
    def __init__(self, source, shuffle, input_data=None):
        ...
        if source == "wiki" and not input_data:
            raise Exception(
                "You need to specify the input data when parsing "
