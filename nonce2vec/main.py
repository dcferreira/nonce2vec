...
def _load_nonce2vec_model(args, info, nonce):
    ...
    if args.replication:
        ...
        if args.sample is None:
            raise Exception(
                "In replication mode you need to specify the " "sample parameter"
            )
        if args.window_decay is None:
            raise Exception(
                "In replication mode you need to specify the " "window_decay parameter"
            )
        if args.sample_decay is None:
            raise Exception(
                "In replication mode you need to specify the " "sample_decay parameter"
            )
...
def _test_on_chimeras(args):  # pylint:disable=R0914
    ...
    logger.info(
        "Testing Nonce2Vec on the chimeras {} dataset containing "
        "{} batches and {} sentences".format(args.on, total_num_batches, total_num_sent)
    )
    num_batch = 1
    ...
    for sentences, nonce, probes, responses in samples:
        ...
        logger.info("-" * 30)
        logger.info("Processing batch {}/{}".format(num_batch, total_num_batches))
...
def _display_stats(ranks, ctx_ents):
    ...
    logger.info("Correlation no rounding = {}".format(_spearman(ctx_ents, ranks)))
    logger.info(
        "Correlation round 6 = {}".format(
            _spearman([round(x, 6) for x in ctx_ents], ranks)
        )
    )
    logger.info(
        "Correlation round 5 = {}".format(
            _spearman([round(x, 5) for x in ctx_ents], ranks)
        )
    )
    logger.info(
        "Correlation round 4 = {}".format(
            _spearman([round(x, 4) for x in ctx_ents], ranks)
        )
    )
    logger.info(
        "Correlation round 3 = {}".format(
            _spearman([round(x, 3) for x in ctx_ents], ranks)
        )
    )
    logger.info(
        "Correlation round 2 = {}".format(
...
def _load_informativeness_model(args):
    if not args.info_model:
        logger.warning(
            "Unspecified --info-model. Using background model "
...
def _test_on_definitions(args):  # pylint:disable=R0914
    ...
    for sentences, nonce, probe in samples:
        ...
        ranks.append(rank)
        if args.with_stats:
            gold_nns = model.most_similar("{}_true".format(nonce), topn=vocab_size)
...
def _check_men(args):
    ...
    for (first, second), human in Samples(source="men", shuffle=False):
        if first not in model.wv or second not in model.wv:
            logger.error(
                "Could not find one of more pair item in model "
...
def _train(args):
    ...
    output_model_filepath = futils.get_model_path(
        args.datadir,
        args.outputdir,
        args.train_mode,
        args.alpha,
...
def main():
    ...
    parser_gensim.add_argument(
        "--num-threads",
        type=int,
        default=1,
        help="number of threads to be used by gensim",
    )
    parser_gensim.add_argument("--alpha", type=float, help="initial learning rate")
    parser_gensim.add_argument("--neg", type=int, help="number of negative samples")
    parser_gensim.add_argument("--window", type=int, help="window size")
    parser_gensim.add_argument("--sample", type=float, help="subsampling rate")
    parser_gensim.add_argument("--epochs", type=int, help="number of epochs")
    parser_gensim.add_argument("--min-count", type=int, help="min frequency count")

    # a shared set of parameters when using informativeness
    parser_info = argparse.ArgumentParser(add_help=False)
    parser_info.add_argument(
        "--info-model", type=str, help="informativeness model path"
    )
    parser_info.add_argument(
        "--sum-filter",
        default=None,
    ...
    parser_train.add_argument(
        "--size", type=int, default=400, help="vector dimensionality"
    )
    parser_train.add_argument(
        "--train-mode", choices=["cbow", "skipgram"], help="how to train word2vec"
    )
    parser_train.add_argument(
        "--outputdir", required=True, help="Absolute path to outputdir to save model"
    )
    ...
    parser_test.add_argument(
        "--lambda", type=float, dest="lambda_den", help="lambda decay"
    )
